# AUTOGENERATED! DO NOT EDIT! File to edit: ../nbs/08_learner.ipynb.

# %% auto 0
__all__ = ['DataLoaders', 'to_cuda', 'Learner', 'Metric', 'Accuracy', 'Loss', 'identity', 'CancelFitException',
           'CancelBatchException', 'CancelEpochException', 'with_cbs', 'Callback', 'CudaCB', 'ProgressCallback',
           'MetricsCB']

# %% ../nbs/08_learner.ipynb 1
...
class with_cbs:
    ...
    def __call__(self, f):
        def _f(o, *args, **kwargs):
            ex = globals()[f'Cancel{self.nm.title()}Exception']
            try:
...
# %% ../nbs/08_learner.ipynb 26
class Callback(fc.GetAttr): _default='learner'

# %% ../nbs/08_learner.ipynb 27
...
class MetricsCB(Callback):
    ...
    def before_epoch(self):
        self.metric.reset()
        self.loss_m.reset()
    def after_epoch(self): print(self.epoch, self.model.training, self.metric(), self.loss_m())
    def after_batch(self):
        self.metric.add_batch(self.preds.argmax(dim=1), self.batch[1])
        self.loss_m.add(self.loss, len(self.batch[0]))
